...
test('can specify options in request', () => {
  ...
  const options = { includeArchived: true, withRights: true };
  const invocation = merchi.Product.get(1, options).then(product =>
    expect(product.name).toBe(testName)
  );
...
test('can fetch product with category and domain', () => {
  ...
  const r = merchi.Product.get(1, { embed: { categories: {}, domain: {} } });
...
test('can fetch product with category and explcit session', () => {
  ...
  mockFetch(
    true,
    { product: { name: testName, categories: [categoryData] } },
    200
  );
...
test('can list products from server', () => {
  ...
  mockFetch(
    true,
    {
      products: [{ product: { name: 'p1' } }, { product: { name: 'p2' } }],
      available: 2,
      count: 2
    },
    200
...
test('can list products from server with category', () => {
  ...
  mockFetch(
    ...
    {
      products: [
        { product: { name: 'p1', categories: categoriesData } },
        { product: { name: 'p2', categories: categoriesData } }
      ],
      available: 2,
      count: 2
    },
